...
public class ProductOperationDao {
    ...
    private ProductAttribute getProductAttributeWithNextExpiryDate(ProductCode productCode, Location location) {
        return (ProductAttribute) getSession().createCriteria(ProductAttribute.class)
                ...
                .add(Restrictions.eq("location", location)).addOrder(Order.asc("expiryDate")).setMaxResults(1)
                .uniqueResult();
    }
...
//                operation.getOperationType().getUuid().equals(OperationConstants.URGENT_REPORT_OPERATION);
        ...
        for (ProductCode productCode : productCodes) {
            ...
            ProductAttribute productAttribute = getProductAttributeWithNextExpiryDate(productCode, operation.getLocation());

            if (operation.getOperationType().getUuid().equals(OperationConstants.DISTRIBUTION_OPERATION)) {
                List<ProductStockStatus> childProductStockStatus = getAllProductStockStatuses(operation.getExchangeLocation(), operation.getProductProgram());
                if (childProductStockStatus != null) {
                    ProductStockStatus childProductCodeInStatus = childProductStockStatus.stream().filter(p -> p.getProductCode().equals(productCode)).findFirst().orElse(null);
                    if (childProductCodeInStatus != null) {
                        ProductOperationFlux flux = operation.getFluxes().stream().filter(p -> p.getProductCode().equals(productCode)).findFirst().orElse(null);
                        if (flux != null) {
                            childProductCodeInStatus.setQuantityInStock(childProductCodeInStatus.getQuantityInStock() + flux.getQuantity());
                            childProductCodeInStatus.setStockDate(new Date());
                            ProductAttribute childProductAttribute = getProductAttributeWithNextExpiryDate(productCode, operation.getExchangeLocation());
                            if (childProductAttribute != null) {
                                childProductCodeInStatus.setExpiryNextDate(childProductAttribute.getExpiryDate());
                            }
                            saveProductStockStatus(childProductCodeInStatus);
                        }
                    }
                }
...
//        }
    ...
    public ProductOperation getLastProductOperation(List<ProductOperationType> operationTypes, ProductCode productCode,
        ...
        Criteria criteria = getSession()
                ...
                .add(Restrictions.eq("o.voided", includeVoided))
                .add(Restrictions.eq("f.productCode", productCode))
                .add(Restrictions.between("o.operationDate", limitDate, beforeDate))
...
//                System.out.println("Urgent report calculating monthly consumption");
                createEmergencyReportMonthlyConsumption(operation);
...
//        }
    ...
    private void createEmergencyReportMonthlyConsumption(ProductOperation operation) {
        List<ProductOperationOtherFlux> otherFluxes = new ArrayList<>();

        ProductOperation previousReport = getLastProductOperation(getProductOperationType(
                        OperationConstants.REPORT_OPERATION),
                operation.getProductProgram(),
                operation.getLocation(),
                true, false
        );
            ...
            for (ProductCode productCode : operation.getProductList()) {
                otherFluxes.add(createOtherFlux(
                        getProductOtherFluxQuantity(
                                ...
                                ReportConstants.MONTHLY_CONSUMPTION),
                        ReportConstants.MONTHLY_CONSUMPTION,
                        productCode,
                        operation.getLocation())
                );
        ...
        operation.addAllOtherFlux(otherFluxes);
    }
    ...
    private Double createProposedQuantity(ProductCode productCode, ProductOperation operation, Double leadTime) {
        double quantity = 0.;
        ProductStockStatus stockStatus = getProductLastStockStatusByProductCode(productCode, operation.getProductProgram(), operation.getLocation());
        if (stockStatus != null && stockStatus.getQuantityInStock() > 0.) {
...
//                    otherFluxes.add(getAverageMonthlyConsumption(productCode, lastConsumptionReportFluxes, inventory.getLocation()));
    ...
    private List<ProductOperationOtherFlux> createEmergencyWithoutInventoryReportOtherFluxes(ProductOperation currentReport) {
        ...
        ProductOperation previousInventory = getLastProductOperation(
                getProductOperationType(OperationConstants.INVENTORY_OPERATION),
                currentReport.getProductProgram(),
                currentReport.getLocation(), true, false, currentReport.getOperationDate());

        if (previousInventory == null) {
            previousInventory = getLastProductOperation(
                    getProductOperationType(OperationConstants.INVENTORY_OPERATION),
                    currentReport.getProductProgram(),
                    currentReport.getLocation().getParentLocation(),
                    true, false, currentReport.getOperationDate());
        }

        if (attribute != null && previousInventory != null) {
            ...
            for (String productUuid : productUuidList) {
                ...
                if (productCode != null) {
                    ...
                    ProductOperation previousDistributionWithProduct = getLastProductOperation(
                            Collections.singletonList(
                                    getProductOperationType(OperationConstants.DISTRIBUTION_OPERATION)
                            ),
                            productCode,
                            previousInventory.getOperationDate(),
                            currentReport.getOperationDate(),
                            currentReport.getProductProgram(),
                            currentReport.getLocation().getParentLocation(),
                            false
...
//                    System.out.println("--------------------------------> " + previousDistributionWithProduct);
                    if (previousDistributionWithProduct != null) {
                        otherFluxes.add(createOtherFlux(
                                getProductFluxQuantity(
                                        productCode,
                                        previousDistributionWithProduct.getFluxes().stream().filter(f -> f.getProductCode().equals(productCode)).collect(Collectors.toList())),
                                ReportConstants.RECEIVED_QUANTITY,
                                productCode,
                                currentReport.getLocation()));
                    } else {
                        otherFluxes.add(createOtherFlux(0., ReportConstants.RECEIVED_QUANTITY, productCode, currentReport.getLocation()));
                    }
    ...
    public ProductStockStatus getProductLastStockStatusByProductCode(ProductCode productCode, ProductProgram program,
                                                                     Location location) {
        Criteria criteria = getSession().createCriteria(ProductStockStatus.class, "s").createAlias("s.productCode", "p");
        criteria.add(Restrictions.eq("s.location", location));
        return (ProductStockStatus) criteria.add(Restrictions.eq("s.productCode", productCode))
                .add(Restrictions.eq("p.program", program)).addOrder(Order.desc("s.stockDate")).setMaxResults(1)
                .uniqueResult();
    }
}
